...
processFolder <- function(folderName) {
  ...
  for (fileName in list) {
	dur <- wavdur(files=fileName, path=folderName)
    row <- data.frame(fileName, 0, 0, dur)
    data <- rbind(data, row)
  }
...
gender <- function(filePath) {
  ...
  # Start with empty data.frame.
  data <- data.frame(fileName, 0, 0, 1)
...
# Load data
children <- processFolder('E:/data/speakergender/dataset/vad/test/c')
males <- processFolder('E:/data/speakergender/dataset/vad/test/m')
females <- processFolder('E:/data/speakergender/dataset/vad/test/f')

# Set labels.
children$label <- 0
males$label <- 1
females$label <- 2
data <- rbind(children, males, females)
data$label <- factor(data$label, labels=c('child', 'male', 'female'))
...
write.csv(data, file='test_vad.csv', sep=',', row.names=F)